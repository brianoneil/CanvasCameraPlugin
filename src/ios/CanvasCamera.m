       ...
       completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
           if (error) {
               ...
               dispatch_sync(queue, ^(void) {
                   ...
                   float actualHeight = srcImg.size.height;
                   float actualWidth = srcImg.size.width;
                   float imgRatio = actualWidth/actualHeight;
                   float maxRatio = 600.0/800.0;

                   if(imgRatio!=maxRatio){
                       if(imgRatio < maxRatio){
                           imgRatio = 800.0 / actualHeight;
                   ...
                   UIImage *resizedImg = [CanvasCamera resizeImage:srcImg toSize:CGSizeMake(actualWidth, actualHeight)];
